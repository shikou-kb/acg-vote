/* ============================================================
   四川大学萌战 —— 实时结果页 (results.js)
   从 LeanCloud 数据库读取实时数据
   ============================================================ */
...
var CHART_COLORS = ["#e94560","#ff6b81","#533483","#7c5cbf","#0f3460","#1a5276","#ffd700","#f39c12","#2ecc71","#1abc9c","#e67e22","#e74c3c","#9b59b6","#3498db","#1dd1a1","#ff9ff3"];


// 页面加载 → 从数据库获取数据
document.addEventListener("DOMContentLoaded", async function () {
    try {
        var characters = await loadFromDB();
...
function drawBarChart(characters) {
    ...
    new Chart(ctx, {
        type: "bar",
        data: {
            labels: top10.map(function(c){ return c.emoji+" "+c.name; }),
            datasets: [{
                label: "票数",
                data: top10.map(function(c){ return c.votes; }),
                backgroundColor: top10.map(function(_,i){ return CHART_COLORS[i]; }),
        ...
        options: {
            ...
            plugins: { legend: { display: false },
                tooltip: { backgroundColor:"#141430", titleColor:"#e94560", bodyColor:"#fff", padding:12, cornerRadius:8, borderColor:"#e94560", borderWidth:1,
                    callbacks: { label: function(ctx){ return " "+ctx.raw.toLocaleString()+" 票"; } }
                }
            },
            scales: {
...
function drawPieChart(characters) {
    var groupTotals = {};
    characters.forEach(function(c){ groupTotals[c.group]=(groupTotals[c.group]||0)+c.votes; });
    var labels = Object.keys(groupTotals).map(function(k){ return GROUP_LABELS[k]||k; });
    var values = Object.values(groupTotals);
    var ctx = document.getElementById("pieChart").getContext("2d");
    new Chart(ctx, {
        type: "doughnut",
        data: { labels:labels, datasets:[{ data:values, backgroundColor:["#e94560","#533483","#0f3460","#ffd700"], borderColor:"#0a0a1a", borderWidth:3, hoverOffset:8 }] },
        options: { responsive:true, plugins:{ legend:{ position:"bottom", labels:{ color:"#f0f0f0", padding:18, font:{size:14}, usePointStyle:true } },
            tooltip:{ backgroundColor:"#141430", titleColor:"#e94560", bodyColor:"#fff", padding:12, cornerRadius:8,
                callbacks:{ label:function(ctx){ var t=ctx.dataset.data.reduce(function(a,b){return a+b;},0); return " "+ctx.raw.toLocaleString()+" 票 ("+((ctx.raw/t)*100).toFixed(1)+"%)"; } }
            }
        }}
    });
}
